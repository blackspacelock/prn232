...
INSERT INTO PersonalRoadmaps (PersonalRoadmapId, ProfileId, CareerRoadmapId, Note, ProgressPercentage, CreatedAt, IsDeleted)
SELECT
    NEWID(),
    p.UserId,
    @RoadmapId,
    N'Seeded personal roadmap for frontend visual testing.',
    0,
    @Now,
    0
FROM Profiles p
WHERE p.IsDeleted = 0
  AND NOT EXISTS (
      SELECT 1
      FROM PersonalRoadmaps pr
      WHERE pr.ProfileId = p.UserId
        AND pr.CareerRoadmapId = @RoadmapId
        AND pr.IsDeleted = 0);

UPDATE PersonalRoadmaps
SET Note = COALESCE(Note, N'Seeded personal roadmap for frontend visual testing.'),
    UpdatedAt = @Now
WHERE CareerRoadmapId = @RoadmapId
  AND IsDeleted = 0;

INSERT INTO NodeProgresses (NodeProgressId, PersonalRoadmapId, RoadmapNodeId, Status, Note, CreatedAt, IsDeleted)
SELECT
    NEWID(),
    pr.PersonalRoadmapId,
    rn.RoadmapNodeId,
    CASE rn.RoadmapNodeId
        WHEN '40000000-0000-0000-0000-000000000001' THEN 4
        WHEN '40000000-0000-0000-0000-000000000002' THEN 4
        WHEN '40000000-0000-0000-0000-000000000003' THEN 1
        WHEN '40000000-0000-0000-0000-000000000004' THEN 2
        WHEN '40000000-0000-0000-0000-000000000005' THEN 1
        WHEN '40000000-0000-0000-0000-000000000010' THEN 4
        WHEN '40000000-0000-0000-0000-000000000011' THEN 1
        WHEN '40000000-0000-0000-0000-000000000015' THEN 3
        ELSE 0
    END,
    CASE rn.RoadmapNodeId
        WHEN '40000000-0000-0000-0000-000000000003' THEN N'Review architecture decision examples.'
        WHEN '40000000-0000-0000-0000-000000000011' THEN N'Practice writing ADRs for project decisions.'
        ELSE NULL
    END,
    @Now,
    0
FROM PersonalRoadmaps pr
INNER JOIN RoadmapNodes rn ON rn.CareerRoadmapId = pr.CareerRoadmapId AND rn.IsDeleted = 0
WHERE pr.CareerRoadmapId = @RoadmapId
  AND pr.IsDeleted = 0
  AND NOT EXISTS (
      SELECT 1
      FROM NodeProgresses np
      WHERE np.PersonalRoadmapId = pr.PersonalRoadmapId
        AND np.RoadmapNodeId = rn.RoadmapNodeId
        AND np.IsDeleted = 0);

UPDATE np
SET Status = CASE np.RoadmapNodeId
        WHEN '40000000-0000-0000-0000-000000000001' THEN 4
        WHEN '40000000-0000-0000-0000-000000000002' THEN 4
        WHEN '40000000-0000-0000-0000-000000000003' THEN 1
        WHEN '40000000-0000-0000-0000-000000000004' THEN 2
...
INNER JOIN (
    SELECT
        np.PersonalRoadmapId,
        CAST(ROUND(CASE WHEN COUNT(*) = 0 THEN 0 ELSE SUM(CASE WHEN np.Status = 4 THEN 1 ELSE 0 END) * 100.0 / COUNT(*) END, 2) AS decimal(5, 2)) AS Percentage
    FROM NodeProgresses np
    WHERE np.IsDeleted = 0
    GROUP BY np.PersonalRoadmapId
) progress ON progress.PersonalRoadmapId = pr.PersonalRoadmapId
WHERE pr.CareerRoadmapId = @RoadmapId
  AND pr.IsDeleted = 0;
...
SELECT
    ...
    (SELECT COUNT(*) FROM PersonalRoadmaps WHERE CareerRoadmapId = @RoadmapId AND IsDeleted = 0) AS PersonalRoadmapCount,
    (SELECT COUNT(*) FROM NodeProgresses np INNER JOIN PersonalRoadmaps pr ON pr.PersonalRoadmapId = np.PersonalRoadmapId WHERE pr.CareerRoadmapId = @RoadmapId AND np.IsDeleted = 0) AS NodeProgressCount;
